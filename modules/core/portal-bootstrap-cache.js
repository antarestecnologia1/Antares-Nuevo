 ...
 * Restore priorizado (opts.deferNonEssential): aplica primero ESSENTIAL_STORAGE_KEYS y

 * repone el resto en requestIdleCallback para acortar trabajo síncrono antes del primer paint.
...
(function registerPortalBootstrapCache() {
  ...
  var deferredRestoreGeneration = 0;



  function bumpDeferredRestoreGeneration() {

    deferredRestoreGeneration += 1;

    return deferredRestoreGeneration;
  ...
  function loadShardedDataForKeyList(userId, meta, keyList) {

    var data = {};

    if (!keyList || !keyList.length) return data;
  ...
  function loadShardedData(userId, meta) {
    ...
    return loadShardedDataForKeyList(userId, meta, keyList);
  ...
  function persistShards(userId, data, extras, opts) {
    ...
    if (!wroteAny && extras === undefined) return true;
  ...
  function scheduleDeferredShardRestore(userId, meta, otherKeys, ticket) {
    ...
    if (!uid || !otherKeys || !otherKeys.length) return;



    function applyRest() {

      if (ticket !== deferredRestoreGeneration) return;

      var restData = loadShardedDataForKeyList(uid, meta, otherKeys);
   ...
   * @param {{ deferNonEssential?: boolean }} [opts] — deferNonEssential: parse/aplica primero claves esenciales.

   */

  function tryRestore(userId, opts) {
    ...
    var ticket = bumpDeferredRestoreGeneration();
    ...
    if (meta) {
      ...
      if (opts.deferNonEssential) {

        var metaKeys = Array.isArray(meta.keys) && meta.keys.length ? meta.keys : allServerBackedKeys();

        var essentialInMeta = [];

        var otherKeys = [];

        for (var mi = 0; mi < metaKeys.length; mi += 1) {

          var mk = metaKeys[mi];

          if (!isServerBackedKey(mk)) continue;

          if (ESSENTIAL_STORAGE_KEY_SET.has(mk)) essentialInMeta.push(mk);

          else otherKeys.push(mk);

        }

        if (otherKeys.length && essentialInMeta.length) {

          data = loadShardedDataForKeyList(uid, meta, essentialInMeta);
          ...
          if (!data || !Object.keys(data).length) {
            ...
            scheduleDeferredShardRestore(uid, meta, otherKeys, ticket);
