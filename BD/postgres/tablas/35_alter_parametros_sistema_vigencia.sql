-- Migración: parametros_sistema — múltiples vigencias por clave (SMMLV, auxilio, UVT…)
-- Ejecutar en bases creadas con 02_parametros_sistema.sql (PK solo en clave).
-- Idempotente: la API también aplica este esquema en ensureSystemParametersSchema().
...
ALTER TABLE parametros_sistema ADD COLUMN IF NOT EXISTS id UUID;
UPDATE parametros_sistema SET id = gen_random_uuid() WHERE id IS NULL;
ALTER TABLE parametros_sistema ALTER COLUMN id SET DEFAULT gen_random_uuid();
ALTER TABLE parametros_sistema ALTER COLUMN id SET NOT NULL;
...
BEGIN
  IF NOT EXISTS (
    ...
    WHERE conrelid = 'parametros_sistema'::regclass AND contype = 'p'
  ) THEN
...
ALTER TABLE parametros_sistema DROP CONSTRAINT IF EXISTS chk_parametros_sistema_vigencia;
ALTER TABLE parametros_sistema
  ADD CONSTRAINT chk_parametros_sistema_vigencia
  CHECK (vigente_hasta IS NULL OR vigente_hasta >= vigente_desde);

COMMENT ON TABLE parametros_sistema IS
  'Parámetros legales Colombia por vigencia: SMMLV, auxilio transporte, UVT, aportes, horas semanales.';
